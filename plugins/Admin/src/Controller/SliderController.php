...
class SliderController extends AppController
{
    ...
    function addslider($intEditId=null,$strCopyStatus=null)
    {
        ...
        if($intEditId>0)
        {
            $rowProductInfo = $this->SkSlider->get($intEditId, ['contain' => []  ]);
        }
          $resCategoryInfo =  $this->SkCategory->find('all')->order(['category_name'=>'ASC']);
         $resTagInfo =  $this->SkTag->find('all')->order(['tag_name'=>'ASC']);
                   $resBrandInfo =  $this->SkBrand->find('all')->order(['brand_name'=>'ASC']);

        $this->viewBuilder()->setLayout('sideBarLayout');
        $strPageTitle='Add Slider';
        $this->set(compact('strPageTitle','rowProductInfo','resCategoryInfo','resTagInfo','resBrandInfo'));
    }
    ...
    function SliderProcessRequest()
    {
        ...
        if($intEditId>0)
        {
            $category = $this->SkSlider->get($intEditId, ['contain' => [] ]);
        }else{
    ...
    { 
        if ($this->request->is(['patch', 'post', 'put'])) 
        {
            ...
            if($intBulkAction==1)
            {
                $this->SkSlider->deleteAll( ['slider_id IN' =>$aryPostData['slider_id']]);
                $this->Flash->success(__('The Slider Deleted Successfully'));
                return $this->redirect(['action' => 'index']);
            }
            if($intBulkAction==2)
            {
                $this->SkSlider->updateAll(['slider_status'=>'1'],['slider_id IN' =>$aryPostData['slider_id']]);
                $this->Flash->success(__('Selected Entry Active Successfully'));
                return $this->redirect(['action' => 'index']);
            }
            if($intBulkAction==3)
            {
                $this->SkSlider->updateAll(['slider_status'=>'0'],['slider_id  IN' =>$aryPostData['slider_id'] ]);
