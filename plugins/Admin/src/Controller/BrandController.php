...
class BrandController extends AppController
{
    ...
    function tagProcessRequest()
    {
    	...
    	if($intEditId>0)
    	{
    	   $category = $this->SkBrand->get($intEditId, ['contain' => [] ]);
    	}else{
    ...
    public function bulkaction()
    {
        if ($this->request->is(['patch', 'post', 'put'])) 
        {
            ...
            if($intBulkAction>0)
            {
                ...
                if($intBulkAction==1)
                {
                    $this->SkBrand->deleteAll( ['brand_id IN' =>$aryPostData['brand_id']]);
                    $this->Flash->success(__('Deleted Successfully'));
                    return $this->redirect(['action' => 'index']);
                }
                if($intBulkAction==2)
                {
                    $this->SkBrand->updateAll(['brand_status'=>'1'],['brand_id IN' =>$aryPostData['brand_id']]);
                    $this->Flash->success(__('Selected Entry Active Successfully'));
                    return $this->redirect(['action' => 'index']);
                }
                if($intBulkAction==3)
                {
                    $this->SkBrand->updateAll(['brand_status'=>'0'],['brand_id  IN' =>$aryPostData['brand_id'] ]);
