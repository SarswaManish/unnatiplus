...
<div class="heading-elements">
    <a href="<?php echo SITE_URL; ?>admin/order/export" class="btn btn-primary"><i class="icon-file-plus position-left"></i> Export As CSV</a>
</div>
</div>
...
<ul>
<?php
    $rowTotalCount =$resCountOrder->find('all',['fields'=>['total'=>'count(*)']])->where(' 1 AND trans_status!=2 AND ( trans_payment_status=1 OR trans_method=\'Cash on Delivery\') ')->first();
     $rowConfirmTotalCount =$resCountOrder->find('all',['fields'=>['total'=>'count(*)'],'conditions'=>['trans_status'=>1]])->first();
     
       $rowDispatchedTotalCount =$resCountOrder->find('all',['fields'=>['total'=>'count(*)'],'conditions'=>['trans_status'=>3]])->first();
          $rowPendingTotalCount =$resCountOrder->find('all',['fields'=>['total'=>'count(*)'],'conditions'=>[' 1 AND trans_status=0 AND ( trans_payment_status=1 OR trans_method=\'Cash on Delivery\') ']])->first();
...
<li class="<?php if($strType=='confirm'){ ?>active<?php } ?>"><a href="<?php echo SITE_URL; ?>admin/order/index/confirm">Delivered </a> <span class="count">(<?php echo  $rowConfirmTotalCount->total; ?>)</span></li>
