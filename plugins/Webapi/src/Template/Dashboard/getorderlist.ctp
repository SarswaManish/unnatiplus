      ...
      <div class="ob-order-details">
        ...
        <?php }?>
         <?php if($rowTransactionList->trans_status==6){ ?>
            ...
            <span class="btn btn-primary text-white">Returned</span>
        <?php }?>
        <?php if($rowTransactionList->trans_status==4){ ?>
            ...
            if($resProductName['product_is_return']  == 1){
                ...
                <div class="btd-id text-center">
                    <a href="javascript:void(0)" style="color:#fff;padding:12px;background-color:#d60008" class="btn btn-danger btn-sm" onclick="sendOtpAndVerify('<?php echo $rowTransactionList->trans_id ?>','<?php echo $rowUserInfo->user_id; ?>','return')">RETURN ORDER</a>
                </div>
            <?php } }?>
        <?php } ?>
         <?php if($rowTransactionList->trans_status==1 && isset($rowUserInfo->user_id) && $rowUserInfo->user_id>0){ ?>
         <div class="btd-id text-center">
            <a href="javascript:void(0)" style="color:#fff;padding:12px;background-color:#d60008" class="btn btn-danger btn-sm" onclick="sendOtpAndVerify('<?php echo $rowTransactionList->trans_id ?>','<?php echo $rowUserInfo->user_id; ?>')">CANCEL ORDER</a>
         </div>
         <?php } ?>
...
<button id="trigger_model" data-toggle="modal" data-target="#verifyOtpplz" style="display:none;">Click</button>
...
<script>
    var base_url = "https://codexosoftware.live/unnatiplus/webapi/";
   function sendOtpAndVerify(orderId,userid,param='cancel'){
           ...
           var datastring = {'for_what':for_what,'user_id':userid};
           $.post(base_url+'dashboard/sendOtp',datastring,function(response){
       ...
       function verifyOtpAndReturn(orderId,userid){
           ...
           $.post(base_url+'dashboard/initiatereturn',datastring,function(response){
               var jsondata = JSON.parse(response);
    ...
    function verifyCancelOtp(orderId,userid){
        var datastring = {'user_otp':$('#cancel_user_otp').val(),'trans_id':orderId,'user_id':userid};
        $('#verifyotp_spinner').show();
        $.post(base_url+'dashboard/cancelorder',datastring,function(response){
            var jsondata = JSON.parse(response);
            if(jsondata.message=='ok'){
               location.reload();
            }else{
